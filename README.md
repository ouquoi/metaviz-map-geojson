...
| **Weight column** | Numeric column used to scale line/border thickness relative to other features. Auto-detected from columns named `weight`, `poids`, `importance`, or `size`. Leave empty to use a fixed **Stroke width** for all features. |
...
- Per-feature line/border thickness scaled by a weight column, relative to the min/max weight across all features
...
| Weight | No | Number | Scales line/border thickness relative to other features; missing/invalid falls back to the minimum stroke width |
